...
from user_simulation.updaters import VanillaMMIUpdater
...
def main():
    ...
    def parse_arguments():
        ...
        parser.add_argument('--margin', '-m', type=float, default=5.0,
                            help='Margin for MMI training (default: 5.0)')
        parser.add_argument('--lam', '-l', type=float, default=2.0,
                            help='Coefficiency for MMI training (default: 2.0)')
        ...
        parser.add_argument('--bias', type=float, default=1.,
                            help='Bias for modified ES beam search.')
    ...
    trainer.extend(
        extensions.PlotReport(
            ['main/loss/total', 'main/loss/target'],
    ...
    trainer.extend(
        extensions.PlotReport(
            ['main/acc/target', 'main/acc/candid'],
            trigger=(args.plot_interval, 'iteration'),
            file_name='acc.png'
        )
    )
    trainer.extend(
        extensions.PrintReport(
            ['elapsed_time', 'epoch', 'iteration', 'main/loss/total', 'main/loss/target', 'main/acc/target']
        ),
        trigger=(args.display_interval, 'iteration')
    )
    trainer.extend(
