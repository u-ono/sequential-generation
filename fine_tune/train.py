...
from fine_tune.updaters import FineTuneUpdater
from fine_tune.extensions import FineTuneEvaluator, visualize
...
def parse_arguments():
    ...
    parser.add_argument('--c_rec', type=float, default=0.5,
                        help='Coeff for KL Divergence.')
    ...
    parser.add_argument('--enc', default='sn_res', type=str,
                        help='Encoder type.')
...
def main():
    ...
    updater = FineTuneUpdater(
        models=(enc, gen, dis, fbr, sim),
        iterator=train_iter,
        optimizer={'enc': opt_enc, 'gen': opt_gen, 'dis': opt_dis, 'fbr': opt_fbr},
        data=train_data,
        device=args.gpu,
        args=args
    )
    ...
    trainer.extend(
        extensions.snapshot_object(enc, 'enc_{.updater.epoch}'),
        trigger=(args.snapshot_interval, 'epoch')
    )
    trainer.extend(
